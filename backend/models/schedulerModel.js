...
SchedulerModel.notifyStatusChange = () => {
  return true;
};
 ...
 * Create notification
 */
SchedulerModel.createNotification = async (
  userId,
  title,
  message,
  description,
 ...
 */
SchedulerModel.getUnreadNotifications = async (userId) => {
  const sql = `
      SELECT *
      FROM Notifications
      WHERE user_id = ?
 ...
 */
SchedulerModel.markAsRead = async (notificationId) => {
  const sql = `
...
SchedulerModel.deleteOldNotifications = async () => {
  await db.query(
...
SchedulerModel.notificationExists = async (userId, title, startDate) => {
